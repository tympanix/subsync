...
class Transcode:
    ...
    def __init__(
        ...
        self.duration = (
            duration if type(duration) is timedelta else timedelta(seconds=duration)
        )
        ...
        self.output = os.path.join(
            tempfile.gettempdir(), "subsync_" + randomString() + ".wav"
        )
    ...
    def __length(self):
        ...
        duration = [
            x.decode("utf-8") for x in cmd.stdout.readlines() if b"Duration" in x
        ]
